...
def save_to_csv(title, ingredients):
    with open("recipes_poissons.csv", mode="a", newline="", encoding="utf-8") as file:
...
# Create a new CSV with headers for "poissons"
with open("recipes_poissons.csv", mode="w", newline="", encoding="utf-8") as file:
    writer = csv.writer(file)
    writer.writerow(["Titre", "Ingrédients", "Instructions"])

# Scrape the first three pages of the "poissons" category
base_url = "https://www.cuisineaz.com/categories/plats/poissons-cat48891"
